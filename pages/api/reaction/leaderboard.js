...
//   milliseconds (lower is better). Submitting a score keeps the LOWER of the
//   stored best and the new value, so the board always reflects a player's
//   personal record. An account that has never submitted a score has `best =
//   null` and ranks last.
//
//   GET  -> { updated, scores: [ {name, best, at} x10 ] }   (best = ms | null)
//   POST -> { action, ... }
//     action "create" : { name, pin }        -> create account (best starts null)
//     action "login"  : { name, pin }        -> verify PIN, return best + board
//     action "score"  : { name, pin, best }  -> verify PIN, keep min(stored, new)
...
const OBJECT_KEY = "leaderboard/reaction.json";
const MAX_ENTRIES = 10;
const MAX_RETRIES = 5;

// Sanity clamp so a tampered client can't post a "best" that is physically
// impossible or absurd. A real human reaction is well above ~100 ms; we allow a
// generous floor and a hard ceiling so the board stays meaningful.
...
function coerceBest(v) {
  ...
  return Math.max(MIN_BEST_MS, Math.min(MAX_BEST_MS, n));
}

// Derive the public top-N board from the account table (ranked by best ms,
// ascending — fastest first; null-best accounts rank last).
function publicBoard(doc) {
  ...
  rows.sort((x, y) => {
    const bx = x.best == null ? Infinity : x.best;
    const by = y.best == null ? Infinity : y.best;
